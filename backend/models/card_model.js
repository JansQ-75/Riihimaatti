...
const card = {
  ...
  getByCardnumberStart: function (cardnumber, callback) {
    return db.query(
      'SELECT card.idcard, cardnumber, card.idcustomer, type, fname, lname, idbank_account FROM card JOIN customer ON card.idcustomer=customer.idcustomer JOIN access_rights a ON a.idcard=card.idcard WHERE cardnumber=?',
      [cardnumber],
      callback,
    );
  },
  ...
  update: function (idcard, card_data, callback) {
    bcrypt.hash(card_data.pin, 10, function (err, hashed_pin) {
      ...
      db.query('SELECT * FROM card WHERE idcard=?', [idcard], (err, result) => {
        ...
        return db.query(
          'UPDATE card SET cardnumber=?, pin=?, idcustomer=?,type=?, locked_status=? WHERE idcard=?',
          [
